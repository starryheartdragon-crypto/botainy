...
export const SoundtrackDrawer: React.FC<SoundtrackDrawerProps> = ({
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-black bg-opacity-40 transition-opacity ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}> 
      <div className="absolute right-0 top-0 h-full w-[400px] bg-white shadow-lg flex flex-col p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">Suggested Soundtrack</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          <h3 className="font-semibold mb-2">All Tracks</h3>
          <ul>
              ...
              <li key={track.youtubeId + idx} className="mb-4 border-b pb-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="ml-2 text-xs text-gray-500">{track.addedBy === 'AI' ? 'AI Suggestion' : 'User Added'}</span>
                  </div>
                  <button
                ...
                <div className="text-sm mt-1">{track.reasoning}</div>
              </li>
            ))}
          </ul>
        </div>
        {selectedTrack && (
          <div className="mt-4">
            ...
            <div className="mt-2 text-sm">{selectedTrack.reasoning}</div>
          </div>
        )}
      </div>
    </div>
